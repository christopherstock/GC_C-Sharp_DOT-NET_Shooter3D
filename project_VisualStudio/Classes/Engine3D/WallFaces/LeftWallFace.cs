/*  $Id: LeftWallFace.cs,v 1.1 2006/11/17 06:46:14 jenetic.bytemare Exp $
 *  =================================================================================
 *  A left wall.
 */
...
{
    public class LeftWallFace : WallFace
    {
        public LeftWallFace( float initX, float initY, float initZ, float initWidth, float initHeight, int initTextureID, float initTilingX, float initTilingY ) : base( ref initTilingX, ref initTilingY, initWidth, initHeight )
        {
            textureID = initTextureID;
            vertices  = new Vertex[]
            {
                new Vertex ( initX,             initY,                  initZ + initWidth,  initTilingX,    0.0f            ),
                new Vertex ( initX,             initY,                  initZ,              0.0f,           0.0f            ),
                new Vertex ( initX,             initY + initHeight,     initZ,              0.0f,           initTilingY     ),
                new Vertex ( initX,             initY + initHeight,     initZ + initWidth,  initTilingX,    initTilingY     ),
            }; //endarray
        } //endconstruct
    } //endclass
} //endnamespace
